...
function ContentInfoModal({ isOpen, onClose, content }) {
  ...
  return isOpen ? (
    ...
    >
      <div
        className="w-1/2 bg-gray-800 bg-opacity-90 rounded-lg shadow-lg overflow-hidden relative"
        ...
        <div className="p-4 text-white bg-dark">
          <div className="flex justify-between text-gray-400 text-sm mb-2">
            {content.type === 'TV Show' ? (
              <div>
                {`${firstAiredYear} - ${lastAiredYear}`} • {numberOfSeasons} seasons
              </div>
            ) : (
              <div>
                {content.release_year}
                {content.runtime && ` • ${content.runtime} minutes`}
              </div>
            )}
