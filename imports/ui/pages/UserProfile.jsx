...
export default function UserProfile() {
  ...
  const { lists, subscribedLists, followUser, unfollowUser, ratings, loading } =
    useTracker(() => {
      ...
      const subscribedHandler = Meteor.subscribe(
        "subscribedLists",
        Meteor.userId()
      );
      ...
      const ratings = RatingCollection.find({
  ...
  const favouritesList = userLists.find(
    (list) => list.listType === "Favourite"
  );
  const toWatchList = userLists.find((list) => list.listType === "To Watch");
  const customWatchlists = userLists.filter(
    (list) => list.listType === "Custom"
  );
