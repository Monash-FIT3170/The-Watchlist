...
Meteor.methods({
  ...
  "ratings.addOrUpdate"({ userId, contentId, contentType, rating }) {
    console.log("addOrUpdate method called with:", {
      userId,
      contentId,
      contentType,
      rating,
...
const createDefaultLists = (userId) => {
  ...
  defaultLists.forEach((list) => {
    Meteor.call(
      "list.create",
      { userId, title: list.title, listType: list.listType, content: [] },
      (error) => {
        if (error) {
...
Meteor.methods({
  "list.subscribe"(listId) {
    if (!this.userId) {
      throw new Meteor.Error(
        "not-authorized",
        "You must be logged in to subscribe to lists."
      );
    }
    ...
    if (list.userId === this.userId) {
      throw new Meteor.Error(
        "invalid-action",
        "You cannot subscribe to your own list."
      );
    }
...
Meteor.methods({
  "list.getSubscribed": function ({ userId }) {
    ...
    if (!this.userId) {
      throw new Meteor.Error(
        "not-authorized",
        "You must be logged in to access subscribed lists."
      );
    }
    ...
    if (this.userId !== userId) {
      throw new Meteor.Error(
        "not-authorized",
        "You are not authorized to view other users' subscribed lists."
      );
    }
...
Meteor.methods({
  "list.unsubscribe"(listId) {
    ...
    if (!this.userId) {
      throw new Meteor.Error(
        "not-authorized",
        "You must be logged in to unsubscribe from lists."
      );
    }
    ...
    if (list.userId === this.userId) {
      throw new Meteor.Error(
        "invalid-action",
        "You cannot unsubscribe from your own list."
      );
    }
...
Meteor.methods({
  "genres.getAll": async function () {
    const movieGenres = await MovieCollection.rawCollection().distinct(
      "genres"
    );
...
Meteor.methods({
  "users.getSimilarUsers"() {
    if (!this.userId) {
      throw new Meteor.Error(
        "not-authorized",
        "You must be logged in to find similar users."
      );
    }
    ...
    const randomUsers = Meteor.users
      .rawCollection()
      .aggregate([{ $sample: { size: 100 } }])
      .toArray();

    const randomUsersPromise = Meteor.wrapAsync((cb) =>
      randomUsers.then((res) => cb(null, res)).catch(cb)
    );
    ...
    const userScores = users.map((user) => {
      ...
      const commonFavourites = userFavouritesArray.filter((favourite) =>
        currentUserFavouritesArray.some(
          (currentFavourite) =>
            currentFavourite.contentId === favourite.contentId
        )
      );
      ...
      const avgListLength =
        (userFavouritesArray.length + currentUserFavouritesArray.length) / 2;
...
Meteor.methods({
  "list.getById": function (listId) {
    ...
    if (!this.userId) {
      ...
      throw new Meteor.Error(
        "not-authorized",
        "You must be logged in to view lists."
      );
    }
...
Meteor.methods({
  updateUsername(newUsername) {
    ...
    const existingUser = Meteor.users.findOne({ username: newUsername });
    if (existingUser) {
      throw new Meteor.Error(
        "username-taken",
